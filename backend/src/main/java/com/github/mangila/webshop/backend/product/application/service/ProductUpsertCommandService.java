...
@Service
public class ProductUpsertCommandService {

    private static final Logger log = LoggerFactory.getLogger(ProductUpsertCommandService.class);

    private final EventServiceGateway eventServiceGateway;
    private final ProductCommandRepository commandRepository;
    private final JsonMapper jsonMapper;

    public ProductUpsertCommandService(EventServiceGateway eventServiceGateway,
    ...
    @Transactional
    public Event execute(ProductUpsertCommand command) {
        var product = commandRepository.upsert(command).orElseThrow(() -> new CommandException(
                command.getClass(),
                ProductDomain.class,
                HttpStatus.CONFLICT,
                "Upsert failed"));
